...
enum Role { doctor, caregiver, patient } // Removed  if you aren't using it
...
class StoredAccount {
  ...
  /// Used after login (/users/me).
  factory StoredAccount.fromJson(Map<String, dynamic> json) {
    return StoredAccount(
      email: json['email'] ?? '',
      password: '', // Backend never returns passwords.
      role: roleFromString(
        (json['role'] ?? 'patient').toString(),
      ),
      name: json['name'] ?? '',
      age: json['age'],
      gender: json['gender'],
      ...
      licenseNumber:
          json['licenseNumber'] ?? json['license_number'],

      linkedPatientId:
          json['linkedPatientId'] ?? json['linked_patient_id'],

      patientId:
          json['patientId'] ?? json['patient_id'],
    );
  }
}
